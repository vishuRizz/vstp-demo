//! Simple VSTP TCP Server - For use with ngrok tunneling
//!
//! Run with: cargo run --example simple_server_ngrok
//!
//! IMPORTANT: Make sure ngrok is running with TCP forwarding:
//!   ngrok tcp 8080
//!
//! This server binds to 0.0.0.0:8080 to accept connections from ngrok
...
fn display_frame_structure(frame: &Frame, encoded_bytes: &[u8]) {
    ...
    println!("   [INTEGRITY] CRC-32 checksum: {} bytes verified", encoded_bytes.len());
    println!("   [EXTENSIBILITY] {} custom headers supported (vs HTTP fixed headers)", frame.headers.len());
    println!("   [OVERHEAD] Frame overhead: {} bytes (HTTP: ~200+ bytes)", encoded_bytes.len() - frame.payload.len());
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🌐 VSTP Server for ngrok tunneling");
    println!("═══════════════════════════════════════════════════════════\n");
    println!("🚀 Starting VSTP Server on 0.0.0.0:8080");
    println!("   Waiting for clients to connect via ngrok...");
    println!("   📊 Server will show binary encoding/decoding of all frames");
    println!("\n💡 Make sure ngrok is running:");
    println!("   ngrok tcp 8080\n");

    // Bind server to 0.0.0.0:8080 to accept external connections via ngrok
    let server = VstpTcpServer::bind("0.0.0.0:8080").await?;
    let local_addr = server.local_addr()?;
    println!("✅ Server bound to: {}\n", local_addr);
