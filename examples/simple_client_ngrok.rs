//! Simple VSTP TCP Client - For use with ngrok tunneling
//!
//! Run with: cargo run --example simple_client_ngrok
//!
//! IMPORTANT: Make sure ngrok is running with TCP forwarding:
//!   ngrok tcp 8080
//!
//! Then use the TCP forwarding address (e.g., tcp://0.tcp.ngrok.io:12345)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🌐 VSTP Client via ngrok");
    println!("═══════════════════════════════════════════════════════════\n");

    // Get ngrok address from user or use default
    println!("Enter ngrok TCP address (e.g., 0.tcp.ngrok.io:12345)");
    println!("Or press Enter to use default from environment variable NGROK_ADDR");
    println!("(Default: localhost:8080 for local testing)");
    println!("\n⚠️  IMPORTANT: Make sure ngrok is running with: ngrok tcp 8080");
    println!("   (Not 'ngrok http 8080' - VSTP needs TCP forwarding!)\n");

    let mut ngrok_addr = String::new();
    io::stdin().read_line(&mut ngrok_addr)?;
    let mut ngrok_addr = ngrok_addr.trim().to_string();

    // Remove tcp:// prefix if present
    if ngrok_addr.starts_with("tcp://") {
        ngrok_addr = ngrok_addr.strip_prefix("tcp://").unwrap().to_string();
    }

    let server_addr = if ngrok_addr.is_empty() {
        // Try environment variable, fallback to localhost
        let env_addr = std::env::var("NGROK_ADDR").unwrap_or_else(|_| "127.0.0.1:8080".to_string());
        if env_addr.starts_with("tcp://") {
            env_addr.strip_prefix("tcp://").unwrap().to_string()
        } else {
            env_addr
        }
    } else {
    ...
    println!("🔌 Connecting to VSTP Server at {}...", server_addr);

    // Connect to server via ngrok
    let mut client = VstpTcpClient::connect(&server_addr).await?;
    println!("✅ Connected to server via ngrok!\n");
